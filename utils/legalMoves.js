...
const getHorizontalMoveList = ({ pos, rank, config, board }) => {
  ...
  config.forEach((item) => {
    ...
    for (let i = 0; i < item.number; i++) {
      ...
      temRank =
        item.moveType === 'left' ? rowMapper[temRank][1] : rowMapper[temRank][2]
      let boardPos = board.find((item) => item.name === newPos)
      if (boardPos) {
        if (boardPos[item.piece]) 
          break;
...
const pMoves = (board, current) => {
  ...
  let allowedMoves = []
  let number = 1
  if(current.white && Number(pos) === 7)
    number = 2
  if(current.black && Number(pos) === 2)
    number = 2
  const vConfig = [
    {
      number,
